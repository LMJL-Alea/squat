...
- Remove dependency to **purrr** in favor of base R `*apply()` family to
  reduce dependency trail.
- Replace **furrr** with **future.apply** for consistency with the
  removal of **purrr**.
...
- Adapted code to match new API in
  [**fdacluster**](https://astamm.github.io/fdacluster/index.html)
  package.
...
- A first API proposal with a class `qts` and a class `qts_sample` for
  which a number of methods are properly implemented.
...
- Enable
  ...
  to generate a QTS sample of size 1 from a single QTS as input
  argument.
- Rename `change_points` argument to the
  [`plot.qts()`](https://lmjl-alea.github.io/squat/reference/plot.qts.md)
  function to better reflect its flexibility.
...
- Added
  ...
  function to remove any discontinuities in QTS due to quaternion flips.
- Any parallelization computation is now handled using the
  [**futureverse**](https://www.futureverse.org) principles and, in
  particular, implemented through the use of the
  [**furrr**](https://furrr.futureverse.org) package.
