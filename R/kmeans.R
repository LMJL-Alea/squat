...
#' @param t An optional list of evaluation grids for the QTS. By default, if you do not supply it, the function assumes that all QTS are evaluated on the same grid.
#' @param k The number of clusters (default: 2).
#' @param iter_max The maximum number of iterations before stopping the k-means algorithm if it failed converging (default: 20).
#' @param nstart The number of random restarts for the initial centers (default: 1000).
#'
#' @return A list with two components: \code{memberships} is an integer vector specifying the membership of each data point and \code{Var} is the sum of within-sum-of-squares.
#' @export
#'
#' @examples
